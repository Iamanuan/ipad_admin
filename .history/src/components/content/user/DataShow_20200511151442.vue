 ...
 export default {
   ...
   methods:{
       async getUserData() {
           ...
           this.userdatas.push(res.data[0].onlineNum === null ? 0 : res.data[0].onlineNum) // 当前在线人数
           this.userdatas.push(res.data[0].maxNum === null ? 0 : res.data[0].maxNum)  // 本月峰值日活
           this.userdatas.push('￥' + (res.data[0].estimateFree === null ? 0 : res.data[0].estimateFree))  // 本月预估费用
           this.userdatas.push('￥' + ( res.data[0].userInfo.balance === null ? 0 :  res.data[0].userInfo.balance)) // 余 额
       }
   }
 }
</script>
